...
int main(int argc, char *argv[]) {
  Flags flags = catReadOptions(argc, argv);
  if (!flags.invalid) {
...
Flags catReadOptions(int argc, char *argv[]) {
  ...
  int currentFlag = 0;
  Flags flags = {false};
  while ((currentFlag = getopt_long(argc, argv, "beEvnstT", longOptions, 0)) !=
         -1) {
    switch (currentFlag) {
      case 'b':
